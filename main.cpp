...
static void linear_ranking_selector( int &seed );
static void linear_ranking_selector2( int &seed );
static void binary_selector( int &seed );
...
//
static void tournament_selector( int &seed )
{
  cout << "tournament_selector" << endl;
  return;
}
//
// linear ranking
...
static void linear_ranking_selector( int &seed )
{
  //cout << "linear_ranking_selector" << endl;
  //SP is between 1.0 and 2.0
  //fitness[pos] = (2-SP) + 2(SP-1)*((pos-1)/(Nid-1))
  int *ranks = new int[POPULATION_SIZE];
  ...
  // now apply roulette wheel selection
  ...
  Fitness_t r = Random_double ( 0, sumS, seed ); 
  ...
  // k is the selected chromosome
  // 
  //
  // Find the 2nd fittest
  //
  // [Select] Generate random number from interval (0,S) - r.
  Fitness_t r2 = Random_double ( 0, (sumS-ranks[k]), seed ); 
  // [Loop] Go through the population and sum fitnesses from 0 - sum s. 
  // When the sum s is greater then r, stop and return the chromosome where you are.
  int k2 = 0;
  Fitness_t sums2 = 0;
